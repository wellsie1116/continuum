...
void DotSceneLoader::processCamera(TiXmlElement *XMLNode, SceneNode *pParent)
{
	...
	if(pElement)
	{
		Real nearDist = getAttribReal(pElement, "nearPlaneDist");
		pCamera->setNearClipDistance(nearDist);

		Real farDist =  getAttribReal(pElement, "farPlaneDist");
...
void DotSceneLoader::processEntity(TiXmlElement *XMLNode, SceneNode *pParent)
{
	...
	while(pElement)
	{
		...
		pParent->getUserObjectBindings().setUserAny(name, Ogre::Any(value));
	...
	try
	{
		...
		pEntity = mSceneMgr->createEntity(pParent->getName(), meshFile);
	...
	onEntityCreated(pEntity, pParent);
}
